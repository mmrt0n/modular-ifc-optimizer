...
| 파일 | 설명 |
|------|------|
| **[IFC최적화.exe](dist/IFC최적화.exe)** | Windows 64비트 실행 파일 (53.2MB) |
...
| 🖥️ 인터랙티브 시뮬레이터 | Python 최적화 결과 통합, 단계별 애니메이션 |
...
| 색상 | 종류 | 설명 |
|------|------|------|
| 🟢 초록 | 온장 | 규격 그대로 부착 (석고보드 900×1800 / 합판 1220×2440) |
...
프로젝트_석고보드_최적화결과.html ← 배치도 + 자재 집계
...
| 항목 | 구현 여부 | 비고 |
|------|-----------|------|
| RTL (오른쪽 끝부터) 배치 | ✅ | Python + JS 동일 |
| 개구부 중심 대칭 (SYM) | ✅ | |
| 각재 피치 450mm (STUD) | ✅ | |
| 바닥 밀착 (FLOOR_GAP=0) | ✅ | M3 확인결과 9-1 |
| 직선절단 / 노치절단 분류 | ✅ | |
| 재사용 풀 (MIN_W=300 / MIN_H=450) | ✅ | Python·JS 양쪽 구현 |
